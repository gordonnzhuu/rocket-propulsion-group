...
int AD7193_driver(void* args){
    ...
    uint8_t CS_pin = (s_args->spi_bus == &SPI)? PB11 : 0;
    uint8_t rdy_pin = (s_args->spi_bus == &SPI)? PB14 : 1;
    ...
    if((s_args->comm_bits & AD7193_REG_READ) && reg_id == AD7193_DATA_REG){
        while(1){
    ...
    if(s_args->comm_bits & AD7193_REG_READ){
        ...
        if(reg_id == AD7193_STATUS_REG || reg_id == AD7193_ID_REG || reg_id == AD7193_GPOCON_REG)
            ...
            sensor_return |= s_args->spi_bus->transfer(0) << 24;
...
int AD7193_wrapper(void* args){
    ...
    if (configs->internal_temp) {
        ...
        channel_bits = (1 << (configs->channel)) << 8;
    }
    ...
    uint32_t config_write_payload = 0x0 | (!configs->differential << AD7193_DIFFERENTIAL_BIT) | channel_bits | (gain_bits) ;
    ...
    {
        driver_args.spi_bus->transfer(0xFF);
    }
    ...
    while(1) {
        ...
        uint32_t mode_reg_payload = 0x080060 | (1 << 20) | (0b001 << 21);
        ...
        uint32_t result = AD7193_driver((void*)&driver_args);
        delay(100);

        //Serial.println(result);
        
        if (!configs->internal_temp) {
            units = AD7193_codeToVolts(result >> 8, gain_bits, false);
        } else {
            units = (result - 0x800000)/2815.0 - 273;
        }

        int channel = result & 0x7F;
        digitalWrite(PB11, HIGH);

        uint32_t t = millis() - dt;
        Serial.println(t);
